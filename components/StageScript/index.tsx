...
const StageScript: React.FC<Props> = ({
  ...
  const [activeTab, setActiveTab] = useState<TabMode>(
    project.scriptData ? 'script' : 'story'
  )
  ...
  const reuseVisualDataFromPrevious = (
    ...
    const previousCharacters = buildReuseLookup(
      previous.characters || [],
      (item) => item.name
    )
    next.characters = (next.characters || []).map((character) => {
      const direct = previousCharacters.byId.get(String(character.id))
      const byName = previousCharacters.byKey.get(
        normalizeAssetKey(character.name)
      )
    ...
    const previousScenes = buildReuseLookup(
      previous.scenes || [],
      (item) => item.location
    )
    next.scenes = (next.scenes || []).map((scene) => {
      const direct = previousScenes.byId.get(String(scene.id))
      const byLocation = previousScenes.byKey.get(
        normalizeAssetKey(scene.location)
      )
    ...
    const previousProps = buildReuseLookup(
      previous.props || [],
      (item) => item.name
    )
  ...
  const isPlaceholderProjectTitle = (value: string): boolean => {
    ...
    if (/^project\s+\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}$/i.test(trimmed))
      return true
    return false
  }
  ...
  const [localLanguage, setLocalLanguage] = useState(
    project.language || DEFAULTS.language
  )
  const [localModel, setLocalModel] = useState(
    project.shotGenerationModel || DEFAULTS.model
  )
  ...
  const [editingShotCharactersId, setEditingShotCharactersId] = useState<
    string | null
  >(null)
  const [editingShotActionId, setEditingShotActionId] = useState<string | null>(
    null
  )
  ...
  const [lastRewriteSnapshot, setLastRewriteSnapshot] = useState<string | null>(
    null
  )
  ...
  useEffect(() => {
    ...
    const draftDuration = getDraftValue(
      localDuration,
      customDurationInput,
      project.targetDuration || DEFAULTS.duration
    )
    ...
    const draftVisualStyle = getDraftValue(
      localVisualStyle,
      customStyleInput,
      project.visualStyle || DEFAULTS.visualStyle
    )
  ...
  const handleInferVisualStyleByImage = async (file: File) => {
    ...
    try {
      ...
      const inferredPrompt = String(
        result.stylePrompt || result.styleLabel || ''
      ).trim()
      if (!inferredPrompt) {
  ...
  const handleAnalyze = async () => {
    ...
    let workingScriptData: ScriptData | null =
      resumeCheckpoint?.scriptData || previousScriptData || null
    ...
    if (resumeCheckpoint?.scriptData) {
      ...
      shouldGenerateOnlyMissingVisuals =
        nextStep === 'structure' && visualsInputStable
    }
    ...
    try {
      updateProject({
        ...
        scriptGenerationCheckpoint: createAnalyzeCheckpoint(
          nextStep,
          analyzeConfigKey,
          workingScriptData
        )
      ...
      if (nextStep === 'structure' || !workingScriptData) {
        ...
        const canReuseVisualData =
          ...
          previousScriptData.visualStyle === finalVisualStyle &&
          previousScriptData.shotGenerationModel === finalModel
        workingScriptData = reuseVisualDataFromPrevious(
          hydrated,
          previousScriptData,
          canReuseVisualData
        )
        workingScriptData = attachGenerationMeta(workingScriptData, {
          structureKey
        })
      ...
      if (nextStep === 'visuals') {
        const visualPassMode = shouldGenerateOnlyMissingVisuals
          ? '增量补全'
          : '全量重建'
        setProcessingMessage(
          `正在生成角色/场景/道具视觉提示词（${visualPassMode}）...`
        )
        ...
        workingScriptData = attachGenerationMeta(hydrated, {
          structureKey,
          visualsKey
        })
        nextStep = 'shots'
      ...
      if (isAbortError(err, controller.signal)) {
        ...
        setError(`错误: ${getErrorMessage(err) || 'AI 连接失败'}`)
      }
  ...
  const handleContinueScript = async () => {
    ...
    const continueBudget =
      SCRIPT_HARD_LIMIT - baseScript.length - separator.length
    ...
    try {
      ...
      if (wasTruncated) {
        showAlert(
          `续写内容已按单集上限自动截断（最大总长 ${SCRIPT_HARD_LIMIT} 字符）。`,
          { type: 'warning' }
        )
      }
    } catch (err: unknown) {
      console.error(err)
      setError(`AI续写失败: ${getErrorMessage(err) || '连接失败'}`)
      try {
  ...
  const handleRewriteScript = async () => {
    ...
    try {
      ...
      const finalContent = (rewrittenContent || streamed)
        .trim()
        .slice(0, SCRIPT_HARD_LIMIT)
      if (!finalContent) {
      ...
      if (wasTruncated || rewrittenContent.length > SCRIPT_HARD_LIMIT) {
        showAlert(
          `改写结果已按单集上限自动截断（最大 ${SCRIPT_HARD_LIMIT} 字符）。`,
          { type: 'warning' }
        )
      }
      ...
      try {
        ...
        const safeRewrittenContent = rewrittenContent
          .trim()
          .slice(0, SCRIPT_HARD_LIMIT)
        ...
        setError(
          `AI改写失败，已恢复原稿: ${getErrorMessage(fallbackErr) || getErrorMessage(streamErr) || '连接失败'}`
        )
      }
    } finally {
  ...
  const handleRewriteSelection = async () => {
    ...
    const baseScript = localScript
    const selectedSegment = baseScript.slice(
    ...
    try {
      ...
      console.error(err)
      setError(`AI选段改写失败: ${getErrorMessage(err) || '连接失败'}`)
      try {
  ...
  const handleRemoveCharacterFromShot = (
    shotId: string,
    characterId: string
  ) => {
    const updatedShots = project.shots.map((shot) => {
  ...
  const handleAddSubShot = (anchorShotId: string) => {
    ...
    const baseShot =
      project.shots.find((shot) => shot.id === groupPrefix) || anchorShot
    const newShot: Shot = {
      id: newId,
    ...
    const insertAt =
      lastIndexInGroup >= 0 ? lastIndexInGroup + 1 : project.shots.length
    const nextShots = [
  ...
  const handleAddShot = (sceneId: string) => {
    ...
    const sceneIndex = project.scriptData.scenes.findIndex(
      (s) => s.id === sceneId
    )
    const lastIndexInScene = project.shots.reduce(
      (idx, shot, i) => (sceneIdsMatch(shot.sceneId, sceneId) ? i : idx),
      -1
    )
    ...
    if (lastIndexInScene >= 0) {
      ...
      for (
        let i = sceneIndex + 1;
        i < project.scriptData.scenes.length;
        i += 1
      ) {
        const nextSceneId = project.scriptData.scenes[i].id
        const nextIndex = project.shots.findIndex((s) =>
          sceneIdsMatch(s.sceneId, nextSceneId)
        )
        if (nextIndex >= 0) {
          insertAt = nextIndex
          break
