...
import { Prop, PropGenerationHistoryItem } from '../../types'
...
const PropCard: React.FC<PropCardProps> = ({
  ...
  useEffect(() => {
    if (showGenerateModal) {
      setPromptDraft(prop.visualPrompt || '')
    }
  }, [showGenerateModal, prop.visualPrompt])
  ...
  const handleConfirmGenerate = () => {
    ...
    if (normalizedPrompt !== (prop.visualPrompt || '')) {
  ...
  const generationHistory: PropGenerationHistoryItem[] =
    prop.generationHistory || []
  ...
  return (
    ...
    >
      {!hasAssetId && (
        <button
          type="button"
          disabled={isSyncingToLibrary || !prop.referenceImage}
          onClick={() =>
            !isSyncingToLibrary && !!prop.referenceImage && onSyncToLibrary()
          }
        ...
        {prop.referenceImage ? (
          <>
            <img
              ...
              onClick={() => onImageClick(prop.referenceImage)}
            />
          </>
        ) : (
            ...
            {generationHistory.length === 0 ? (
              ...
              <div className="flex gap-2 h-[74px] overflow-x-auto overflow-y-hidden pb-1">
                {generationHistory.map((item, index) => {
                  const isActiveHistoryItem = item.imageUrl === prop.referenceImage
                  return (
                    <div
                      key={item.id}
