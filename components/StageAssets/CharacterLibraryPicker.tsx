...
const CharacterLibraryPicker: React.FC<Props> = ({
  isOpen,
  onClose,
  project,
  ...
  const chars = project.characterLibrary.filter(
    (c) => !query.trim() || c.name.toLowerCase().includes(query.toLowerCase())
  )

  const isAlreadyLinked = (charId: string) =>
    existingCharacterIds.includes(charId)
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-2xl bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded-xl overflow-hidden max-h-[80vh] flex flex-col"
        ...
        <div className="px-6 py-3 border-b border-[var(--border-subtle)]">
          <div className="relative">
            ...
            <input
              ...
              placeholder="搜索角色..."
              className="w-full pl-9 pr-3 py-2 bg-[var(--bg-primary)] border border-[var(--border-primary)] text-xs text-[var(--text-primary)] placeholder:text-[var(--text-muted)] outline-none rounded"
              autoFocus
            />
          </div>
        </div>
                ...
                return (
                  ...
                  >
                    <div className="aspect-video bg-[var(--bg-elevated)] relative">
                      {char.referenceImage ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                          <Users className="w-6 h-6 text-[var(--text-muted)] opacity-30" />
                        </div>
                      )}
                      {linked && (
                        <div className="absolute top-2 right-2 p-1 bg-[var(--success-bg)] rounded">
                    ...
                    <div className="p-3">
                      ...
                      {linked && (
                        <div className="text-[9px] text-[var(--success-text)] mt-1 font-mono">
                          已添加
                        </div>
                      )}
                    </div>
