...
export const STYLES = {
  ...
  cardDark:
    'bg-[var(--bg-primary)] p-4 rounded-xl border border-[var(--border-primary)]',
  ...
  smallButton:
    'px-3 py-1.5 bg-[var(--bg-hover)] text-[var(--text-secondary)] hover:bg-[var(--border-secondary)] rounded text-[10px] font-bold transition-all border border-[var(--border-secondary)] flex items-center gap-1',

  // 输入框样式
  input:
    'w-full bg-[var(--bg-surface)] border border-[var(--border-primary)] rounded px-3 py-2 text-xs text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:border-[var(--border-secondary)]',
  textarea:
    'w-full bg-[var(--bg-surface)] border border-[var(--border-primary)] rounded px-3 py-2 text-xs text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:border-[var(--border-secondary)] resize-none',

  // 图片容器样式
  imageContainer:
    'aspect-video bg-[var(--bg-elevated)] relative rounded-lg overflow-hidden cursor-pointer',
  imagePreview: 'w-full h-full object-cover',

  // 标签样式
  badge:
    'px-2 py-1 bg-[var(--bg-elevated)] border border-[var(--border-primary)] rounded text-[10px] text-[var(--text-tertiary)] font-mono uppercase',

  // 模态框样式
  modalOverlay:
    'absolute inset-0 z-50 bg-[var(--bg-base)]/95 flex items-center justify-center backdrop-blur-sm animate-in fade-in duration-200',
  modalContainer:
    'bg-[var(--bg-surface)] border border-[var(--border-primary)] w-full max-w-4xl max-h-[90vh] rounded-2xl flex flex-col shadow-2xl overflow-hidden',
  modalHeader:
    'h-16 px-8 border-b border-[var(--border-primary)] flex items-center justify-between shrink-0 bg-[var(--bg-elevated)]',
