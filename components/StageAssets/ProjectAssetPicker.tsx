...
const ProjectAssetPicker: React.FC<Props> = ({
  isOpen,
  onClose,
  project,
  assetType,
  ...
  const items = isScene
    ? project.sceneLibrary.filter(
        (s) =>
          !query.trim() ||
          s.location.toLowerCase().includes(query.toLowerCase())
      )
    : project.propLibrary.filter(
        (p) =>
          ...
          {items.length === 0 ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {items.map((item) => {
                ...
                const img = isScene
                  ? (item as Scene).referenceImage
                  : (item as Prop).referenceImage
                    ...
                    <div className="aspect-video bg-[var(--bg-elevated)] relative">
                      {img ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                          <Icon className="w-6 h-6 text-[var(--text-muted)] opacity-30" />
                        </div>
                      )}
                      {linked && (
