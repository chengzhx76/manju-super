...
const ShotWorkbench: React.FC<ShotWorkbenchProps> = ({
  ...
  const availableCharacters =
    scriptData?.characters.filter((c) => !shot.characters.includes(c.id)) || []
  const activeProps = (scriptData?.props || []).filter((p) =>
    (shot.props || []).includes(p.id)
  )
  ...
  const [localVideoModelId, setLocalVideoModelId] =
    useState(currentVideoModelId)
  ...
  const isSectionOpen = (sectionKey: SectionKey) =>
  ...
  const modelRouting = resolveVideoModelRouting(
    localVideoModelId || currentVideoModelId || 'sora-2'
  )
  ...
  const hasSameGridLayout =
    !!nineGrid?.panels?.length &&
    existingGridLayout.panelCount === selectedGridLayout.panelCount
  ...
  const getCheckLabel = (checkKey: string, fallback: string) =>
    checkLabelMap[checkKey] || fallback
  ...
  const weakestCheck = quality?.checks?.length
    ...
    : undefined
  const qualityActionHint = weakestCheck
    ? adviceMap[weakestCheck.key] || ''
    : ''
    ...
    () => [
      {
        ...
        label: '1 资产绑定',
        done: !!scene && activeCharacters.length > 0
      },
      {
        key: 'narrative' as const,
        label: '2 动作台词',
        done: hasActionSummary
      },
      {
          ...
          {isSectionOpen('quality') && quality && (
            <div className="px-4 pb-4 border-t border-[var(--border-primary)] space-y-2">
              <div className="pt-3 flex items-center justify-between gap-2">
                <span
                  className={`px-2 py-1 rounded-md text-[10px] font-mono border ${qualityBadgeClass}`}
                >
              ...
              <p className="text-[10px] text-[var(--text-muted)]">
                来源：{qualitySourceLabel} · 评分时间：
                {new Date(quality.generatedAt).toLocaleString()}
              </p>
              <p className="text-[10px] text-[var(--text-muted)]">
                注：这里显示的是总分和等级，不是“warning条数”。点击每项右侧 ?
                可查看评分依据。
              </p>
                ...
                {quality.checks.map((check) => (
                  <div key={check.key} className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span
                        className={`w-16 text-[10px] font-mono ${check.passed ? 'text-[var(--success-text)]' : 'text-[var(--warning-text)]'}`}
                      >
                        {check.score}/100
                      </span>
                      <span
                        className="flex-1 text-[11px] text-[var(--text-tertiary)] truncate"
                        title={check.details || check.label}
                      >
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setExpandedCheckKey((prev) =>
                            prev === check.key ? null : check.key
                          )
                        }
            ...
            <div className="border-t border-[var(--border-primary)] p-3 space-y-3">
              ...
              {effectiveVideoInputMode === 'keyframes' ? (
                <KeyframeEditor
                  ...
                  canCopyNext={
                    shotIndex < totalShots - 1 && nextShotHasStartFrame
                  }
                ...
                <div className="space-y-2">
                  ...
                  {!hasSameGridLayout && nineGrid?.panels?.length ? (
                    <p className="text-[10px] text-[var(--text-muted)]">
                      当前已有 {existingGridLayout.label}{' '}
                      结果，点击上方按钮将重新生成 {selectedGridLayout.label}。
                    </p>
                  ) : null}
