...
const normalizeDisplayAmount = (value: number) =>
...
const getPlanQuotaUsd = (
  plan: NewApiSubscriptionPlan | undefined,
  status: NewApiStatus | null
) => {
...
const getTopupDiscountValue = (
  discountMap: Record<string, number> | undefined,
  ...
  const matchedEntry = Object.entries(discountMap).find(
    ([key]) => Number(key) === numericAmount
  )
...
const formatTopupDiscountLabel = (discount: number | null) => {
  ...
  if (
    !Number.isFinite(foldValue) ||
    foldValue <= 0 ||
    Math.abs(foldValue - 10) < 0.0001
  )
    return null
...
const formatSubscriptionDuration = (
  plan: NewApiSubscriptionPlan | undefined
) => {
...
const formatSubscriptionResetPeriod = (
  plan: NewApiSubscriptionPlan | undefined
) => {
...
const formatSubscriptionEndPrefix = (
  status: string | undefined,
  isExpired: boolean
) => {
...
const StatusPill: React.FC<{ active?: boolean; children: React.ReactNode }> = ({
  active = false,
  children
}) => (
  <span
    className={`inline-flex items-center border px-2 py-0.5 text-[9px] font-mono uppercase tracking-wider ${active ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-400' : 'border-[var(--border-primary)] text-[var(--text-tertiary)]'}`}
  >
    {children}
  </span>
)
...
export const BillingPanel: React.FC<BillingPanelProps> = ({
  ...
  const selectedTopupDiscountLabel = formatTopupDiscountLabel(
    getTopupDiscountValue(topupDiscountMap, topupAmount)
  )
  const onlineTopupMethods = (
    topupInfo?.enable_online_topup === false ? [] : topupMethods
  ).filter((method) => !['stripe', 'creem'].includes(method.type))
  const walletPayMethods = topupInfo?.enable_online_topup
    ? onlineTopupMethods
    : []
  ...
  if (
    ...
    subscriptionPlans.some((item) => item.plan?.stripe_price_id)
  ) {
    paymentMethodTypes.add('stripe')
  }
  if (
    topupInfo?.enable_creem_topup &&
    subscriptionPlans.some((item) => item.plan?.creem_product_id)
  ) {
    ...
    <div className="space-y-6">
      ...
      >
        <div className="grid gap-4 md:grid-cols-3">
          <StatCard
            label="当前余额"
            value={formatQuotaInUsd(session.user?.quota, status)}
            hint="支付完成后可在这里立即确认到账"
          />
          <StatCard
            label="累计消耗"
            value={formatQuotaInUsd(session.user?.used_quota, status)}
            hint="帮助你判断近期使用强度"
          />
          <StatCard
            label="支付方式"
            value={paymentMethodTypes.size || 0}
            hint={
              topupInfoLoading || subscriptionLoading
                ? '正在同步支付渠道'
                : '当前可选支付渠道数量'
            }
          />
      ...
      {(subscriptionLoading || hasSubscriptionPlans || hasAnySubscription) && (
        <SectionCard
          title="订阅充值"
          description="套餐有效期、额度重置、总额度和当前订阅展示与 new-api 页保持一致。"
        >
          {subscriptionLoading ? (
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {activeSubscriptions.length > 0 ? (
                      <StatusPill active>
                        {activeSubscriptions.length} 个生效中
                      </StatusPill>
                    ) : (
                      <StatusPill>无生效</StatusPill>
                    )}
                  ...
                  <div className="flex items-center gap-2">
                    <StatusPill>
                      {formatBillingPreference(billingPreference)}
                    </StatusPill>
                    <button
                    ...
                    >
                      <RefreshCcw
                        className={`w-4 h-4 ${subscriptionLoading ? 'animate-spin' : ''}`}
                      />
                    </button>
                  </div>
                </div>
                ...
                {hasAnySubscription ? (
                  <div className="mt-4 space-y-4 border-t border-[var(--border-primary)] pt-4">
                    {allSubscriptions.map((item, index) => {
                      ...
                      const title = subscription?.plan_id
                      ...
                      return (
                        <div
                          ...
                          className={
                            index > 0
                              ? 'border-t border-[var(--border-primary)] pt-4'
                              : ''
                          }
                        >
                          <div className="flex flex-col gap-2 text-sm sm:flex-row sm:items-center sm:justify-between">
                            ...
                            {isActive && (
                              <span className="text-[var(--text-tertiary)]">
                                剩余 {getRemainingDays(item)} 天
                              </span>
                            )}
                          </div>

                          <div className="mt-2 text-sm text-[var(--text-tertiary)]">
                            {formatSubscriptionEndPrefix(
                              subscription?.status,
                              isExpired
                            )}{' '}
                          ...
                          <div className="mt-2 text-sm text-[var(--text-tertiary)]">
                            ...
                            {totalAmount > 0 ? (
                              <>
                                {formatUsdQuotaValue(usedAmount, status)}/
                                {formatUsdQuotaValue(totalAmount, status)} ·
                                剩余 {formatUsdQuotaValue(remainAmount, status)}{' '}
                                美金额度
                                <span className="ml-2">
                                  已用 {getUsagePercent(item)}%
                                </span>
                              </>
                            ) : (
                              '不限'
              ...
              {hasSubscriptionPlans ? (
                <div className="grid gap-4 lg:grid-cols-2 2xl:grid-cols-3">
                  {subscriptionPlans.map((item, index) => {
                    ...
                    const originalRmbPrice = getPlanOriginalRmbPrice(
                      plan,
                      status
                    )
                    const discountText = getPlanDiscountText(plan, status)
                    const availableMethods = [
                      ...(plan.stripe_price_id && topupInfo?.enable_stripe_topup
                        ? [{ key: 'stripe', label: 'Stripe' }]
                        : []),
                      ...(plan.creem_product_id && topupInfo?.enable_creem_topup
                        ? [{ key: 'creem', label: 'Creem' }]
                        : []),
                      ...walletPayMethods.map((method) => ({
                        key: method.type,
                        label: method.name
                      }))
                    ]
                    ...
                    return (
                      <div
                        key={plan.id}
                        className={`flex h-full flex-col border bg-[var(--bg-secondary)] p-5 ${isRecommended ? 'border-[var(--accent)] shadow-[0_0_0_1px_rgba(99,102,241,0.35)]' : 'border-[var(--border-primary)]'}`}
                      >
                        <div className="min-h-[72px]">
                          <div className="flex flex-wrap items-center gap-2">
                            {isRecommended && (
                              <span className="bg-[var(--accent-bg)] px-2 py-0.5 text-[9px] font-mono uppercase tracking-wider text-[var(--accent-text)]">
                                推荐
                              </span>
                            )}
                            <div className="text-2xl font-semibold leading-tight text-[var(--text-primary)] break-words">
                              {plan.title || '订阅套餐'}
                            </div>
                          </div>
                          {plan.subtitle && (
                            <div className="mt-2 line-clamp-2 text-sm leading-6 text-[var(--text-tertiary)]">
                              {plan.subtitle}
                            </div>
                          )}
                        </div>

                        <div className="mt-6 text-4xl font-semibold text-[var(--text-primary)]">
                          {formatSubscriptionPrice(plan, status)}
                        </div>
                          ...
                          <div className="mt-2 flex flex-wrap items-center gap-2 text-sm text-[var(--text-tertiary)]">
                            {originalRmbPrice !== null && (
                              <span className="line-through">
                                原价 {formatRmbAmount(originalRmbPrice)}
                              </span>
                            )}
                            {discountText && (
                              <span className="border border-emerald-500/30 bg-emerald-500/10 px-2 py-0.5 text-[9px] font-mono text-emerald-400">
                                {discountText}
                              </span>
                            )}
                            ...
                            <div className="flex flex-wrap gap-2">
                              {availableMethods.map((method) => (
                                <button
                                  ...
                                  onClick={() =>
                                    void onSubscriptionPay(plan.id, method.key)
                                  }
                ...
                <EmptyState
                  title="当前没有可购买订阅套餐"
                  description="如果你已经在 new-api 后端配置了订阅商品，请确认当前账号具备查看套餐的权限。"
                />
              ) : null}
            </div>
          )}
      ...
      <div className="grid gap-6 xl:grid-cols-[1.08fr_0.92fr]">
        <SectionCard
          title="在线充值"
          description="金额会自动预估，无需额外点击计算，适合直接给钱包补充额度。"
        >
          {topupInfoLoading ? (
            ...
            <EmptyState
              title="当前没有可用钱包充值方式"
              description="如果你使用的是自建 new-api 实例，请先在后端配置在线支付渠道；订阅套餐和兑换码仍可在本页继续处理。"
            />
          ) : (
            <div className="space-y-5">
              ...
              {amountOptions.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {amountOptions.map((amount) => {
                    ...
                    const discountLabel = formatTopupDiscountLabel(
                      getTopupDiscountValue(topupDiscountMap, amount)
                    )
                    ...
                    return (
                      ...
                      >
                        ...
                        {discountLabel && (
                          <span
                            className={`mt-1 text-[10px] leading-none ${isSelected ? 'opacity-80' : 'text-[var(--text-tertiary)]'}`}
                          >
                            {discountLabel}
                          </span>
                        )}
              ...
              <div className="border border-[var(--border-primary)] bg-[var(--bg-secondary)] px-4 py-4">
                ...
                <div className="mt-2 text-2xl font-semibold text-[var(--text-primary)]">
                  {estimateLoading
                    ? '自动计算中…'
                    : formatPayableAmount(payableAmount, status)}
                </div>
                <div className="mt-2 text-sm text-[var(--text-tertiary)] leading-6">
                  {estimateError ||
                    '如果你的后端设置了不同的币种和汇率，这里会自动按照当前 EndPoint 的展示规则计算。'}
                </div>
              </div>
            </div>
          )}
        ...
        <div className="space-y-6">
          ...
          <SectionCard
            title="使用提示"
            description="按下面步骤完成在线充值或兑换码兑换，金额会自动更新。"
          >
            <ul className="space-y-3 text-xs leading-relaxed text-[var(--text-tertiary)]">
              <li>
                -
                在线充值：先选择支付方式，再选择快捷金额或手动输入充值数量，确认无误后点击“立即支付”。
              </li>
              <li>
                -
                金额说明：输入或切换充值数量后，下方“预估支付金额”会自动计算，无需额外操作。
              </li>
              <li>
                -
                兑换码使用：已有兑换码时，直接在右上方输入兑换码并点击“立即兑换”。
              </li>
              <li>
                -
                购买兑换码：如果还没有兑换码，可先点击“购买兑换码”，购买完成后返回此处兑换到账。
              </li>
              {topupInfo?.min_topup !== undefined && (
                <li>- 当前最小充值数量：{topupInfo.min_topup}</li>
              )}
            </ul>
          </SectionCard>
        </div>
