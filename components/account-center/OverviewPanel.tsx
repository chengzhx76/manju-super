    ...
    <div className="space-y-6">
      ...
      >
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <StatCard
            label="当前余额"
            value={formatQuotaInUsd(user?.quota, status)}
            valueClassName="break-all"
            hint="用于当前账号可消费额度"
          />
          <StatCard
            label="累计消耗"
            value={formatQuotaInUsd(user?.used_quota, status)}
            valueClassName="break-all"
            hint="便于快速判断最近使用情况"
          />
          <StatCard
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        ...
        <SectionCard
          title="账号信息"
          description="只保留用户真正关心的账号资料，不展示后台接入细节。"
        >
          <div className="grid gap-4 md:grid-cols-2">
            <div className="min-w-0 border border-[var(--border-primary)] bg-[var(--bg-secondary)] p-4">
              ...
              <div className="break-words text-2xl font-light text-[var(--text-primary)]">
                {user?.display_name || session.username}
              </div>
              <div className="mt-2 text-[10px] text-[var(--text-tertiary)] font-mono leading-relaxed">
                登录后在当前站点内直接管理账户能力
              </div>
            </div>
            <div className="min-w-0 border border-[var(--border-primary)] bg-[var(--bg-secondary)] p-4">
              ...
              <div className="break-all text-2xl font-light text-[var(--text-primary)]">
                {user?.email || '未绑定邮箱'}
              </div>
              <div className="mt-2 text-[10px] text-[var(--text-tertiary)] font-mono leading-relaxed">
                用于注册验证、找回密码或接收通知
