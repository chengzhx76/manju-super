...
const inputClassName =
  'border border-[var(--border-primary)] bg-[var(--bg-secondary)] px-4 py-3 text-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] outline-none transition-colors focus:border-[var(--border-secondary)]'
const EMPTY_TEXT = '—'
...
const TaskProgress: React.FC<{ status?: string; progress?: string }> = ({
  status,
  progress
}) => {
...
export const LogsPanel: React.FC<LogsPanelProps> = ({
  ...
  const taskSummary = tasks.reduce(
    (summary, task) => {
      ...
      if (['SUBMITTED', 'QUEUED', 'IN_PROGRESS'].includes(taskState))
        summary.running += 1
      return summary
    },
    ...
    <div className="space-y-6">
      <SectionCard
        ...
        action={
          ...
          >
            {(logView === 'usage' ? logsLoading : tasksLoading) ? (
      ...
      {logView === 'usage' ? (
        <>
          <div className="grid gap-4 md:grid-cols-3">
            <StatCard
              label="消费额度"
              value={formatQuota(logStats?.quota, status)}
              hint="当前筛选条件下的累计消费金额。"
            />
            <StatCard
              label="RPM"
              value={logStats?.rpm ?? 0}
              hint="每分钟请求速率。"
            />
            <StatCard
              label="TPM"
              value={logStats?.tpm ?? 0}
              hint="每分钟 Token 消耗量。"
            />
          </div>
          ...
          <SectionCard
            title="日志明细"
            description="保留时间、令牌、模型和输入输出，便于快速确认消费来源。"
          >
            {logsLoading ? (
              ...
              <EmptyState
                title="暂无日志"
                description="可以先调整时间范围、令牌名称、模型名称或渠道 ID，再重新查询。"
              />
            ) : (
              <div className="max-h-[70vh] overflow-auto border border-[var(--border-primary)]">
                <table className="min-w-full text-sm">
                  <thead className="sticky top-0 z-10 bg-[var(--bg-secondary)]">
                    <tr>
                      <th className="px-4 py-3 text-left text-[10px] font-mono uppercase tracking-widest text-[var(--text-muted)]">
                        时间
                      </th>
                      <th className="px-4 py-3 text-left text-[10px] font-mono uppercase tracking-widest text-[var(--text-muted)]">
                        令牌
                  ...
                  <tbody>
                    {logs.map((log) => (
                      <tr
                        key={log.id}
                        className="border-t border-[var(--border-primary)] align-top"
                      >
                        <td className="whitespace-nowrap px-4 py-3">
                          {formatDateTime(log.created_at)}
                        </td>
                        <td className="px-4 py-3">
                          {log.token_name || EMPTY_TEXT}
                        </td>
                        <td className="px-4 py-3">
                          {log.model_name || EMPTY_TEXT}
                        </td>
                        <td className="whitespace-nowrap px-4 py-3">
                          {log.prompt_tokens ?? 0} /{' '}
            ...
            {logTotal > logPageSize && (
              <div className="mt-5 flex items-center justify-between text-[10px] font-mono text-[var(--text-muted)] uppercase tracking-widest">
                <span>
                  第 {logPage} / {usageTotalPages} 页，共 {logTotal} 条
                </span>
                <div className="flex gap-2">
                  <button
                    onClick={() =>
                      void onUsagePageChange(Math.max(1, logPage - 1))
                    }
                    disabled={logPage <= 1}
                  ...
                  <button
                    onClick={() =>
                      void onUsagePageChange(
                        Math.min(usageTotalPages, logPage + 1)
                      )
                    }
        ...
        <>
          <div className="grid gap-4 md:grid-cols-4">
            ...
            <StatCard
              label="本页成功"
              value={taskSummary.success}
              hint="当前列表中已成功完成的任务。"
            />
            <StatCard
              label="本页进行中"
              value={taskSummary.running}
              hint="当前列表中处于排队或执行中的任务。"
            />
            <StatCard
              label="本页失败"
              value={taskSummary.failure}
              hint="当前列表中执行失败的任务。"
            />
          </div>
          ...
          <SectionCard
            title="任务明细"
            description="支持查看异步任务的状态、进度、耗时和结果链接。"
          >
            {tasksLoading ? (
              <div className="flex min-h-[240px] items-center justify-center text-[var(--text-tertiary)]">
                <Loader2 className="h-5 w-5 animate-spin" />
              ...
              <EmptyState
                title="暂无任务"
                description="可以先调整时间范围、任务 ID、平台或状态，再重新查询。"
              />
            ) : (
              <div className="max-h-[70vh] overflow-auto border border-[var(--border-primary)]">
                <table className="min-w-full text-sm">
                  ...
                  <tbody>
                    {tasks.map((task) => {
                      ...
                      return (
                        ...
                        >
                          <td className="whitespace-nowrap px-4 py-3">
                            {formatDateTime(task.submit_time)}
                          </td>
                          <td className="whitespace-nowrap px-4 py-3">
                            {formatDateTime(task.finish_time)}
                          </td>
                          <td className="whitespace-nowrap px-4 py-3">
                            {formatTaskDuration(
                              task.submit_time,
                          ...
                          <td className="whitespace-nowrap px-4 py-3">
                            <span
                              className={`inline-flex px-2 py-0.5 text-[9px] font-mono uppercase tracking-wider ${statusMeta.className}`}
                            >
                              {statusMeta.label}
                            </span>
                          </td>
                          ...
                          <td className="px-4 py-3">
                            {resultUrl ? (
                              ...
                              <span
                                className="block max-w-[240px] truncate text-rose-400"
            ...
            {taskTotal > taskPageSize && (
              <div className="mt-5 flex items-center justify-between text-[10px] font-mono text-[var(--text-muted)] uppercase tracking-widest">
                <span>
                  第 {taskPage} / {taskTotalPages} 页，共 {taskTotal} 条
                </span>
                <div className="flex gap-2">
                  <button
                    onClick={() =>
                      void onTaskPageChange(Math.max(1, taskPage - 1))
                    }
                    disabled={taskPage <= 1}
                    className="border border-[var(--border-primary)] px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-[var(--text-tertiary)] hover:text-[var(--text-primary)] hover:border-[var(--border-secondary)] transition-colors disabled:opacity-40"
                  >
                  ...
                  <button
                    onClick={() =>
                      void onTaskPageChange(
                        Math.min(taskTotalPages, taskPage + 1)
                      )
                    }
