...
const LarkDirector: React.FC<Props> = ({ project, updateProject, onGeneratingChange }) => {
      ...
      <div className="h-16 border-b border-[var(--border-primary)] bg-[var(--bg-elevated)] px-6 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-4">
          <h2 className="text-lg font-bold text-[var(--text-primary)] flex items-center gap-3">
            ...
            导演工作台(小云雀版)
            <span className="text-xs text-[var(--text-muted)] font-mono font-normal uppercase tracking-wider bg-[var(--bg-base)]/30 px-2 py-1 rounded">Director Workbench</span>
          </h2>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1">
              <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs transition-all bg-[var(--accent)] text-[var(--text-primary)] cursor-pointer" title="横屏 (1280x720)">
                <Monitor className="w-4 h-4" />
                <span>横屏</span>
              </button>
              <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs transition-all bg-[var(--bg-hover)] text-[var(--text-tertiary)] hover:bg-[var(--border-secondary)] hover:text-[var(--text-secondary)] cursor-pointer" title="竖屏 (720x1280)">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-[var(--bg-base)]/30 border border-[var(--border-primary)]">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <input className="w-3.5 h-3.5 rounded border-[var(--border-secondary)] bg-[var(--bg-hover)] text-[var(--accent)] focus:ring-2 focus:ring-[var(--accent)] focus:ring-offset-0 cursor-pointer" type="checkbox" />
            </label>
          </div>
          <span className="text-xs font-mono px-2 py-1 rounded border text-amber-300 border-amber-500/40 bg-amber-500/10">质检分 70</span>
          <span className="text-xs text-[var(--text-tertiary)] mr-4 font-mono">1 / 12 完成</span>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {project.scriptData?.characters.map((char) => (
                <div key={char.id} className="flex flex-col gap-1.5 cursor-pointer group">
                  ...
                  <div className="text-[10px] text-[var(--text-secondary)] text-center truncate">{char.name}-基础形象</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {project.scriptData?.scenes.map((scene) => (
                <div key={scene.id} className="flex flex-col gap-1.5 cursor-pointer group">
                  ...
                  <div className="text-[10px] text-[var(--text-secondary)] text-center truncate">{scene.location}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex-1 flex min-h-0 border-b border-[var(--border-primary)]">
          ...
          <div className="w-[360px] bg-black shrink-0 relative flex flex-col">
            <div className="flex-1 relative flex items-center justify-center">
              {activeClip?.videoUrl ? (
                <video src={activeClip.videoUrl} className="w-full h-full object-cover" controls={false} />
              ) : (
                <div className="text-[var(--text-muted)] text-xs">视频预览区</div>
              )}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 to-transparent flex items-center gap-3">
                ...
                <div className="text-white text-[10px] font-mono">00:01 / 00:10</div>
