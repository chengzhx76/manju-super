...
export default forwardRef((props: any, ref) => {
  ...
  const bottomActions = canAddFromLibrary && !libraryExpanded
    ? [
        {
          key: 'character',
          title: '角色库',
  ...
  useImperativeHandle(ref, () => ({
    onKeyDown: ({ event }: { event: KeyboardEvent }) => {
      ...
      if (event.key === 'ArrowLeft') {
        if (libraryExpanded) {
          setLibraryExpanded(false);
          const fallbackIdx = bottomActions.findIndex(action => action.filter === libraryFilter);
            ...
            >
              <div className="w-6 h-6 rounded-full overflow-hidden bg-gray-100 shrink-0 flex items-center justify-center border border-gray-200">
                {item.image ? (
                  ...
                  <span className="text-[10px] text-gray-500">{(item.name || '?').slice(0, 1)}</span>
                )}
              </div>
              <div className="flex flex-col overflow-hidden">
                <span className="text-[14px] leading-5 font-medium text-gray-900 truncate">{item.name}</span>
                <span className="text-[11px] leading-4 text-gray-400 truncate">{item.desc}</span>
              </div>
            </button>
          ))
        ) : (
          <div className="px-3 py-2 text-sm text-[var(--text-muted)]">未找到结果</div>
        )}
      </div>
            ...
            return (
              <button
                ...
                className={`w-full text-left flex items-center justify-between gap-2.5 px-2.5 py-2 rounded-xl transition-colors ${
                  selectedIndex === actionIndex ? 'bg-[#edf5ff]' : 'hover:bg-gray-50'
                }`}
              ...
              >
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="flex flex-col overflow-hidden">
                    <span className="text-[14px] leading-5 font-medium text-gray-900 truncate">{action.title}</span>
                    <span className="text-[11px] leading-4 text-gray-400 truncate">{action.subtitle}</span>
                  </div>
                </div>
