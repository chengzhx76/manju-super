...
export function buildProjectLibraryMentionItems(projectLibrary: any, query = ''): MentionItem[] {
...
export default function getSuggestion(getProject: () => any, getProjectLibrary?: () => any) {
  return {
    char: '@',
    items: ({ query }: { query: string }) => {
      const scriptItems = buildMentionItems(getProject(), query);
      const libraryItems = buildProjectLibraryMentionItems(getProjectLibrary?.(), query);
    ...
    render: () => {
      ...
      return {
        onStart: (props: any) => {
          const libraryItems = buildProjectLibraryMentionItems(getProjectLibrary?.(), '');
