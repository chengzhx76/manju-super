...
const ModelList: React.FC<ModelListProps> = ({ type, onRefresh }) => {
  ...
  const handleAddModel = (model: Omit<ModelDefinition, 'id' | 'isBuiltIn'>) => {
    try {
      ...
      showAlert(error instanceof Error ? error.message : '添加模型失败', {
        type: 'error'
      })
    }
  }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-[var(--accent-bg)] border border-[var(--accent-border)] rounded-lg p-3">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs font-bold text-[var(--accent-text-hover)]">
            当前使用
          </span>
        </div>
        {(() => {
          ...
          const provider = activeModel
            ? getProviderById(activeModel.providerId)
            : null
          return (
            <p className="text-[11px] text-[var(--text-secondary)]">
              <span className="font-medium">
                {activeModel?.name || '未选择'}
              </span>
              {provider && (
      ...
      <div className="bg-[var(--bg-hover)]/50 border border-[var(--border-secondary)] rounded-lg p-3 flex items-start gap-2">
        ...
        <p className="text-[10px] text-[var(--text-tertiary)] leading-relaxed">
          点击「使用此模型」按钮可切换激活模型。自定义模型配置了独立提供商后，API
          请求会发送到对应的地址。 点击展开按钮可调整模型参数。
        </p>
      </div>
