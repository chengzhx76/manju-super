...
import {
  ...
  saveSeries,
  createNewSeries,
  deleteSeries as deleteSeriesFromDB,
  getEpisodesByProject,
  getEpisodesBySeries,
  saveEpisode,
  loadEpisode,
  createNewEpisode,
  deleteEpisode as deleteEpisodeFromDB
...
interface ProjectContextValue {
  ...
  updateEpisode: (
    updates: Partial<Episode> | ((prev: Episode) => Episode)
  ) => void
...
export function ProjectProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateProjectLibrary = useCallback(
    (updater: (prev: SeriesProject) => SeriesProject) => {
      setProject((prev) => {
        if (!prev) return null
  ...
  const syncLinkedEpisodes = useCallback(
    (params: {
      ...
      if (syncedEpisodes.length > 0) {
        void Promise.all(syncedEpisodes.map((ep) => saveEpisode(ep))).catch(
          (e) => {
  ...
  const updateCharacterInLibrary = useCallback(
    (character: Character) => {
      if (!project) return

      const existing = project.characterLibrary.find(
        (c) => c.id === character.id
      )
      if (!existing) return
      ...
      syncLinkedEpisodes({
        shouldSyncEpisode: (episode) => {
          if (
            !episode.scriptData?.characters.some(
              (c) => c.libraryId === character.id
            )
          )
            return false
          const ref = (episode.characterRefs || []).find(
            (r) => r.characterId === character.id
          )
          return ref?.syncStatus !== 'local-only'
        },
        syncEpisode: (episode) =>
          syncCharacter(episode, updatedProject, character.id),
  ...
  const removeCharacterFromLibrary = useCallback(
    (characterId: string) => {
      updateProjectLibrary((prev) => {
        return {
          ...prev,
          characterLibrary: prev.characterLibrary.filter(
            (c) => c.id !== characterId
          ),
          lastModified: Date.now()
        }
  ...
  const addSceneToLibrary = useCallback(
    (scene: Scene) => {
      updateProjectLibrary((prev) => {
  ...
  const updateSceneInLibrary = useCallback(
    (scene: Scene) => {
      if (!project) return
      ...
      syncLinkedEpisodes({
        shouldSyncEpisode: (episode) => {
          if (!episode.scriptData?.scenes.some((s) => s.libraryId === scene.id))
            return false
          const ref = (episode.sceneRefs || []).find(
  ...
  const removeSceneFromLibrary = useCallback(
    (sceneId: string) => {
      updateProjectLibrary((prev) => {
        return {
          ...prev,
  ...
  const addPropToLibrary = useCallback(
    (prop: Prop) => {
      updateProjectLibrary((prev) => {
  ...
  const updatePropInLibrary = useCallback(
    (prop: Prop) => {
      if (!project) return
      ...
      syncLinkedEpisodes({
        shouldSyncEpisode: (episode) => {
          if (
            !(episode.scriptData?.props || []).some(
              (p) => p.libraryId === prop.id
            )
          )
            return false
  ...
  const removePropFromLibrary = useCallback(
    (propId: string) => {
      updateProjectLibrary((prev) => {
        return {
          ...prev,
  ...
  const handleCreateSeries = useCallback(
    async (title: string): Promise<Series> => {
  ...
  const handleCreateEpisode = useCallback(
    async (seriesId: string, title: string): Promise<Episode> => {
      if (!project) throw new Error('No project loaded')
  ...
  const syncCharacterToEpisode = useCallback(
    (characterId: string) => {
  ...
  const getOutdatedCharacters = useCallback((): EpisodeCharacterRef[] => {
    ...
    return (currentEpisode.characterRefs || []).filter((ref) => {
      ...
      const libChar = project.characterLibrary.find(
  ...
  const syncSceneToEpisode = useCallback(
    (sceneId: string) => {
      if (!project || !currentEpisode?.scriptData) return
  ...
  const syncPropToEpisode = useCallback(
    (propId: string) => {
      if (!project || !currentEpisode?.scriptData) return
