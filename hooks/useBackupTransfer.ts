...
export const globalBackupFileName = (timestamp: string) =>
  `bigbanana_backup_${timestamp}.json`
...
const resolveMessages = (
  messages?: BackupTransferMessages
): ResolvedBackupTransferMessages => ({
...
export function useBackupTransfer<TPayload = unknown>({
  ...
  const handleImportFileChange = async (
    ...
    try {
      ...
      const projectCount =
        payload?.stores?.seriesProjects?.length ||
        payload?.stores?.projects?.length ||
        0
      const assetCount = payload?.stores?.assetLibrary?.length || 0
      const confirmMessage = resolvedMessages.importConfirm(
        projectCount,
        assetCount
      )
      ...
      showAlert(confirmMessage, {
        ...
        onConfirm: async () => {
          try {
            ...
            showAlert(resolvedMessages.importSuccess(result), {
              type: 'success'
            })
          } catch (error) {
