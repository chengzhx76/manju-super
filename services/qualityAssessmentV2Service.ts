...
const buildShotAssessmentContext = (
  shot: Shot,
  ...
  const characters = (shot.characters || []).map((charId) => {
    ...
    const variation = variationId
      ? char?.variations?.find((entry) => entry.id === variationId)
      : undefined
    return {
      id: charId,
...
const normalizeChecks = (
  rawChecks: LLMRawCheck[] | undefined
): QualityCheck[] => {
  ...
  return CHECK_DEFINITIONS.map((definition) => {
    ...
    const details =
      truncateText(raw?.details, 420) || '信息不足，模型未返回详细依据。'
    return {
      key: definition.key,
