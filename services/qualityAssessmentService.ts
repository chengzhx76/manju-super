...
const evaluatePromptReadiness = (shot: Shot): QualityCheck => {
  ...
  return pickCheck('prompt-readiness', 'Prompt Readiness', score, 30, details)
}

const evaluateAssetCoverage = (
  shot: Shot,
  ...
  const charScoreParts = charIds.map((charId) => {
    const char = scriptData.characters.find(
      (entry) => String(entry.id) === String(charId)
    )
    if (!char) {
    ...
    if (variationId) {
      const variation = char.variations?.find(
        (entry) => entry.id === variationId
      )
      if (variation?.referenceImage) {
        charDetails.push(
          `${char.name}(${variation.name})：有变体参考图（25分）`
        )
        return 25
      }
    }
  ...
  const characterScore = charScoreParts.length
    ? charScoreParts.reduce((sum, value) => sum + value, 0) /
      charScoreParts.length
    : 20
  ...
  const propScoreParts = props.map((propId) => {
    const prop = scriptData.props?.find(
      (entry) => String(entry.id) === String(propId)
    )
    if (!prop) {
  ...
  const propScore = propScoreParts.length
    ? propScoreParts.reduce((sum, value) => sum + value, 0) /
      propScoreParts.length
    : 10
  ...
  return pickCheck('asset-coverage', 'Asset Coverage', totalScore, 20, details)
}
...
const evaluateVideoExecution = (shot: Shot): QualityCheck => {
  ...
  return pickCheck('video-execution', 'Video Execution', score, 20, details)
}
...
const evaluateContinuity = (shot: Shot): QualityCheck => {
  ...
  if (supportsEndFrame && hasCharacters && !endFrame?.imageUrl)
    charEndPenalty = -10
  ...
  return pickCheck('continuity-risk', 'Continuity Risk', score, 10, details)
}
...
const buildSummary = (
  checks: QualityCheck[],
  grade: ShotQualityAssessment['grade']
): string => {
