...
export const fetchNewApiStatus = async (
  endpoint = getNewApiEndpoint()
): Promise<NewApiStatus> => {
  const payload = await proxyFetch<NewApiEnvelope<NewApiStatus>>(
    `/api/new-api/status${buildQueryString({ endpoint })}`
  )
  return unwrapEnvelope(payload)
}

export const bootstrapNewApiSession = async (
  endpoint = getNewApiEndpoint()
): Promise<NewApiSession | null> => {
  const payload = await proxyFetch<NewApiEnvelope<NewApiUser | null>>(
    `/api/new-api/session${buildQueryString({ endpoint })}`
  )
...
export const sendNewApiVerificationCode = async (
  email: string,
  endpoint = getNewApiEndpoint()
): Promise<void> => {
  const payload = await proxyFetch<NewApiEnvelope<null>>(
...
export const loginNewApiUser = async (
  ...
  const payload = await proxyFetch<NewApiEnvelope<NewApiAuthUser>>(
    '/api/new-api/session/login',
    {
      method: 'POST',
      body: JSON.stringify({ endpoint, ...input })
    }
...
export const verifyNewApiTwoFactor = async (
  code: string,
  endpoint = getNewApiEndpoint()
): Promise<NewApiLoginResult> => {
  const payload = await proxyFetch<NewApiEnvelope<NewApiAuthUser>>(
    '/api/new-api/session/2fa',
    {
      method: 'POST',
...
export const logoutNewApiUser = async (): Promise<void> => {
  try {
    await proxyFetch<NewApiEnvelope<null>>('/api/new-api/session/logout', {
      method: 'POST'
    })
  } finally {
    saveSession(null)
...
export const getNewApiSelf = async (
  endpoint = getNewApiEndpoint()
): Promise<NewApiUser> => {
  const payload =
    await proxyFetch<NewApiEnvelope<NewApiUser>>('/api/new-api/self')
...
export const getNewApiTopupInfo = async (): Promise<NewApiTopupInfo> => {
  const payload = await proxyFetch<NewApiEnvelope<NewApiTopupInfo>>(
    '/api/new-api/topup/info'
  )
  return unwrapEnvelope(payload)
}

export const getNewApiSubscriptionPlans = async (): Promise<
  NewApiSubscriptionPlanItem[]
> => {
  const payload = await proxyFetch<
    NewApiEnvelope<NewApiSubscriptionPlanItem[]>
  >('/api/new-api/subscription/plans')
  return unwrapEnvelope(payload) || []
}

export const getNewApiSubscriptionSelf =
  async (): Promise<NewApiSubscriptionSelf> => {
    const payload = await proxyFetch<NewApiEnvelope<NewApiSubscriptionSelf>>(
      '/api/new-api/subscription/self'
    )
    return unwrapEnvelope(payload) || {}
  }

export const requestNewApiAmount = async (amount: number): Promise<number> => {
  const payload = await proxyFetch<NewApiEnvelope<string | number>>(
    '/api/new-api/amount',
    {
      method: 'POST',
...
export const requestNewApiPay = async (
  amount: number,
  paymentMethod: string
): Promise<{ url: string; params: Record<string, string> }> => {
  const payload = await proxyFetch<NewApiEnvelope<Record<string, string>>>(
    '/api/new-api/pay',
    {
      method: 'POST',
...
export const requestNewApiSubscriptionStripePay = async (
  planId: number
): Promise<{ pay_link?: string }> => {
  const payload = await proxyFetch<NewApiEnvelope<{ pay_link?: string }>>(
    '/api/new-api/subscription/stripe/pay',
    {
      method: 'POST',
...
export const requestNewApiSubscriptionCreemPay = async (
  planId: number
): Promise<{ checkout_url?: string }> => {
  const payload = await proxyFetch<NewApiEnvelope<{ checkout_url?: string }>>(
    '/api/new-api/subscription/creem/pay',
    {
      method: 'POST',
      body: JSON.stringify({ plan_id: planId })
    }
...
export const requestNewApiSubscriptionEpayPay = async (
  planId: number,
  paymentMethod: string
): Promise<{ url: string; params: Record<string, string> }> => {
  const payload = await proxyFetch<NewApiEnvelope<Record<string, string>>>(
    '/api/new-api/subscription/epay/pay',
    {
      method: 'POST',
...
export const redeemNewApiCode = async (code: string): Promise<number> => {
  const payload = await proxyFetch<NewApiEnvelope<number>>(
    '/api/new-api/topup',
    {
      method: 'POST',
...
export const getNewApiTokens = async (
  page = 1,
  pageSize = 10
): Promise<NewApiPage<NewApiToken>> => {
  const payload = await proxyFetch<NewApiEnvelope<NewApiPage<NewApiToken>>>(
    `/api/new-api/tokens${buildQueryString({ p: page, size: pageSize })}`
  )
  return unwrapEnvelope(payload)
}

export const createNewApiToken = async (
  input: NewApiTokenCreatePayload
): Promise<void> => {
  const payload = await proxyFetch<NewApiEnvelope<null>>(
    '/api/new-api/tokens',
    {
...
export const updateNewApiTokenStatus = async (
  tokenId: number,
  status: number
): Promise<void> => {
  const payload = await proxyFetch<NewApiEnvelope<NewApiToken>>(
    `/api/new-api/tokens/${tokenId}/status`,
    {
      method: 'PATCH',
...
export const getNewApiLogs = async (
  query: NewApiLogsQuery
): Promise<NewApiPage<NewApiLog>> => {
  const payload = await proxyFetch<NewApiEnvelope<NewApiPage<NewApiLog>>>(
    `/api/new-api/logs${buildQueryString({
      p: query.page ?? 1,
...
export const getNewApiLogsStat = async (
  query: Omit<NewApiLogsQuery, 'page' | 'pageSize' | 'requestId'>
): Promise<NewApiLogStats> => {
  const payload = await proxyFetch<NewApiEnvelope<NewApiLogStats>>(
    `/api/new-api/logs/stat${buildQueryString({
      type: query.type ?? 2,
      channel: query.channelId,
      token_name: query.tokenName,
      model_name: query.modelName,
...
export const getNewApiTasks = async (
  query: NewApiTasksQuery
): Promise<NewApiPage<NewApiTask>> => {
  const payload = await proxyFetch<NewApiEnvelope<NewApiPage<NewApiTask>>>(
    `/api/new-api/tasks${buildQueryString({
      p: query.page ?? 1,
      page_size: query.pageSize ?? 20,
