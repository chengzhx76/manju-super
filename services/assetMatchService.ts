...
>(
  ...
  for (const item of items) {
    ...
    const totalScore =
      baseScore + bonusImage + bonusVersion + bonusPrompt + extra
...
function pickBestCharacterMatch(
  aiChar: Character,
  library: Character[]
): Character | null {
  return pickBestByName(library, (lib) => lib.name, aiChar.name, {
    minScore: 0.5
  })
}
...
function pickBestSceneMatch(aiScene: Scene, library: Scene[]): Scene | null {
  ...
  return pickBestByName(library, (lib) => lib.location, aiScene.location, {
    minScore: 0.48,
    extraScore: (lib) => {
      let score = 0
      if (aiTime && aiTime === normalize(lib.time || '')) score += 0.08
      if (aiAtmosphere && aiAtmosphere === normalize(lib.atmosphere || ''))
        score += 0.06
      return score
    }
  })
...
function pickBestPropMatch(aiProp: Prop, library: Prop[]): Prop | null {
  ...
  return pickBestByName(library, (lib) => lib.name, aiProp.name, {
    minScore: 0.45,
    extraScore: (lib) => {
      let score = 0
      if (aiCategory && aiCategory === normalize(lib.category || ''))
        score += 0.08
      if (aiDescription && lib.description) {
        score += Math.min(
          0.1,
          nameSimilarity(aiDescription, lib.description) * 0.15
        )
      }
      return score
    }
...
export function findAssetMatches(
  scriptData: ScriptData,
  project: SeriesProject
): AssetMatchResult {
  const characters: AssetMatchItem<Character>[] = scriptData.characters.map(
    (aiChar) => {
  ...
  const props: AssetMatchItem<Prop>[] = (scriptData.props || []).map(
    (aiProp) => {
  ...
  const newCharacters = matches.characters.map((m) => {
    if (m.reuse && m.libraryAsset) {
      ...
      return {
        ...
        variations: (lib.variations || m.aiAsset.variations || []).map((v) => ({
          ...v
        })),
        turnaround: lib.turnaround
          ? { ...lib.turnaround }
          : m.aiAsset.turnaround,
        status: libImage
          ? ('completed' as const)
          : lib.status || m.aiAsset.status,
        libraryId: lib.id,
