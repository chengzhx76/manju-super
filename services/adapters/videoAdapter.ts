...
const callSyncChatVideoApi = async (
  ...
  // 清理图片数据
  const cleanStart =
    options.startImage?.replace(/^data:image\/(png|jpeg|jpg);base64,/, '') || ''
  const cleanEnd =
    options.endImage?.replace(/^data:image\/(png|jpeg|jpg);base64,/, '') || ''
...
const callSoraApi = async (
  ...
  const references = [options.startImage, options.endImage].filter(
    Boolean
  ) as string[]
  const resolvedModel = apiModel || 'sora-2'
  const isSoraCompatibleModel = isSoraCompatibleVideoModel(resolvedModel)
  const useReferenceArray = resolvedModel
    .toLowerCase()
    .startsWith('veo_3_1-fast')

  if (isSoraCompatibleModel && references.length >= 2) {
    console.warn(
      '⚠️ Capability routing: sora-2 only supports start-frame reference. End-frame reference will be ignored.'
    )
    references.splice(1)
  }
  ...
  while (Date.now() - startTime < maxPollingTime) {
    ...
    if (status === 'completed' || status === 'succeeded') {
      ...
      throw new Error(
        `视频生成失败: ${statusData.error || statusData.message || '未知错误'}`
      )
    }
  }
  ...
  for (let attempt = 1; attempt <= maxDownloadRetries; attempt++) {
    try {
      ...
      const downloadResponse = await fetch(
        `${apiBase}/v1/videos/${videoId}/content`,
        {
          method: 'GET',
          headers: {
            Accept: '*/*',
      ...
      console.warn(`⚠️ 下载出错: ${getErrorMessage(error)}，重试中...`)
      await new Promise((resolve) => setTimeout(resolve, 5000 * attempt))
    }
  }
