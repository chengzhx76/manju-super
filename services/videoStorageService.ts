...
const getVideoDirectory = async (): Promise<FileSystemDirectoryHandle> => {
  ...
  videoDirectoryPromise = storage
    .getDirectory()
    .then((root) =>
      root.getDirectoryHandle(VIDEO_DIRECTORY_NAME, { create: true })
    )
    .catch((error) => {
...
export const persistVideoReference = async (
  ...
  try {
    ...
    console.warn(
      'Persist video to OPFS failed, fallback to original value.',
      error
    )
    return value
  }
...
export const resolveVideoPlaybackSrc = async (
  value: string
...
export const materializeProjectVideosForExport = async (
  project: ProjectState
