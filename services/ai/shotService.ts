...
type NineGridPanelParsePayload = {
  panels?: Array<{
    index?: number
    shotSize?: string
    cameraAngle?: string
...
type NineGridTranslationPayload = {
  translations?: Array<{
    index?: number
...
type KeyframeOptimizePayload = {
  startFrame?: string
  endFrame?: string
}

type SplitShotKeyframe = {
  type: 'start' | 'end'
  visualPrompt: string
}
...
type SplitShotItem = {
  ...
  visualFocus: string
  keyframes: SplitShotKeyframe[]
}

type SplitShotPayload = {
  subShots?: SplitShotItem[]
}
...
const parseAndValidateNineGridPanels = (
  ...
  if (rawPanels.length !== expectedPanelCount) {
    throw new Error(
      `AI返回的panel数量为 ${rawPanels.length}，必须为 ${expectedPanelCount}`
    )
  }

  const usedIndexes = new Set<number>()
  const normalizedPanels = rawPanels.map((p) => {
    ...
    if (rawIndex < 0 || rawIndex >= expectedPanelCount) {
      throw new Error(
        `panel.index 超出范围（当前 ${rawIndex}，要求 0-${expectedPanelCount - 1}）`
      )
    }
  ...
  const invalidPanel = orderedPanels.find(
    (p) => !p.shotSize || !p.cameraAngle || !p.description
  )
  if (invalidPanel) {
    throw new Error(
      'AI返回的panel字段不完整（shotSize/cameraAngle/description 不能为空）'
    )
  }
  ...
  if (invalidLengthPanel) {
    ...
    throw new Error(
      `panel description 词数超出范围（当前 ${words}，要求 10-30）`
    )
  }
  ...
  for (const panel of orderedPanels) {
    ...
    if (seenViewCombos.has(combo)) {
      throw new Error(
        `存在重复视角组合：${panel.shotSize}/${panel.cameraAngle}`
      )
    }
  ...
  if (uniqueShotSizes < requiredShotSizeKinds) {
    throw new Error(
      `shotSize 多样性不足（当前 ${uniqueShotSizes}，至少 ${requiredShotSizeKinds}）`
    )
  }
...
const parseNineGridTranslations = (
  ...
  const parsed = parseJsonWithRecovery<NineGridTranslationPayload>(
    responseText,
    { translations: [] }
  )
  ...
  if (rawTranslations.length !== expectedPanelCount) {
    throw new Error(
      `翻译数量异常：当前 ${rawTranslations.length}，应为 ${expectedPanelCount}`
    )
  }

  const seenIndexes = new Set<number>()
  const normalized = rawTranslations.map((item) => {
    ...
    if (index < 0 || index >= expectedPanelCount) {
      throw new Error(
        `translations.index 超出范围（当前 ${index}，要求 0-${expectedPanelCount - 1}）`
      )
    }
...
export const optimizeBothKeyframes = async (
  ...
  console.log(
    '🎨 optimizeBothKeyframes 调用 - 同时优化起始帧和结束帧 - 使用模型:',
    model
  )
  ...
  try {
    ...
    const parsed = parseJsonWithRecovery<KeyframeOptimizePayload>(result, {})
...
export const optimizeKeyframePrompt = async (
  ...
  const frameSpecificRequirementsTemplate =
    frameType === 'start'
      ? `
- 建立清晰的初始场景和人物状态
- 为即将发生的动作预留视觉空间和动势
...
`
  ...
  try {
    const result = await retryOperation(() =>
      chatCompletion(prompt, model, 0.7, 1024)
    )
    const duration = Date.now() - startTime
...
export const generateActionSuggestion = async (
  ...
  const normalizedDuration = Math.max(
    2,
    Math.min(20, Math.round(targetDurationSeconds * 10) / 10)
  )
...
`
  ...
  try {
    const result = await retryOperation(() =>
      chatCompletion(prompt, model, 0.8, 2048)
    )
    const duration = Date.now() - startTime
...
export const splitShotIntoSubShots = async (
  ...
  model: string = 'gpt-5.2',
  promptTemplates?: PromptTemplateConfig
): Promise<{ subShots: SplitShotItem[] }> => {
...
${String(shot.dialogue).trim()}`
  ...
  const prompt = renderPromptTemplate(template, {
    ...
    sceneAtmosphere: sceneInfo.atmosphere,
    characters:
      characterNames.length > 0 ? characterNames.join('。') : '无特定角色',
    styleDesc,
  ...
  try {
    ...
    const parsed = parseJsonWithRecovery<SplitShotPayload>(result, {})

    if (
      !parsed.subShots ||
      !Array.isArray(parsed.subShots) ||
      parsed.subShots.length === 0
    ) {
    ...
    console.log(
      `✅ 镜头拆分成功，生成 ${parsed.subShots.length} 个子镜头，耗时:`,
      duration,
      'ms'
    )
...
export const enhanceKeyframePrompt = async (
  ...
  try {
    const result = await retryOperation(() =>
      chatCompletion(prompt, model, 0.6, 1536)
    )
    const duration = Date.now() - startTime
...
const buildStoryboardGridPromptContext = (
  ...
  const layoutSpecificConstraint =
    layout.panelCount === 6
...
export const generateNineGridPanels = async (
  ...
  const systemPrompt = renderPromptTemplate(splitSystemTemplate, {
  ...
  const userPrompt = renderPromptTemplate(splitUserTemplate, {
    panelCount: layout.panelCount,
  ...
  try {
    ...
    try {
      ...
      console.warn(
        `⚠️ ${layout.label}首次解析不符合规范，尝试自动纠偏重试:`,
        getErrorMessage(parseError)
      )
      const repairPrompt = `${fullPrompt}

你上一次输出不符合要求（原因：${getErrorMessage(parseError)}）。
...
5) 只输出 JSON，不要任何解释文字`
      ...
      const repairedText = await retryOperation(() =>
        chatCompletion(repairPrompt, resolvedModel, 0.4, 4096, 'json_object')
      )
    ...
    throw new Error(`${layout.label}视角拆分失败: ${getErrorMessage(error)}`)
  }
}
...
export const translateNineGridPanels = async (
  ...
  try {
    ...
    throw new Error(`九宫格翻译失败: ${getErrorMessage(error)}`)
  }
}
...
请严格重新输出 JSON，并保证所有规则完全满足。`
    ...
    throw new Error(`九宫格改写失败: ${getErrorMessage(error)}`)
  }
}
...
export const generateNineGridImage = async (
  ...
  const layout = resolveStoryboardGridLayout(
    options?.panelCount || panels.length
  )
  const gridPromptContext = buildStoryboardGridPromptContext(layout)
  const {
    gridLayout,
  ...
  if (panels.length !== layout.panelCount) {
    throw new Error(
      `网格图片生成前校验失败：panels 数量为 ${panels.length}，必须为 ${layout.panelCount}`
    )
  }

  const panelDescriptions = panels
    .map((panel, idx) =>
      renderPromptTemplate(imagePanelTemplate, {
        index: idx + 1,
  ...
  const nineGridPrompt = `${renderPromptTemplate(imagePrefixTemplate, {
    gridLayout,
    panelCount: layout.panelCount,
    rowCount,
    columnCount,
...
${renderPromptTemplate(imageSuffixTemplate, {
  gridLayout,
  panelCount: layout.panelCount,
  rowCount,
  columnCount,
...
${imageNoTextConstraintTemplate}`
  ...
  try {
    ...
    throw new Error(`${layout.label}图片生成失败: ${getErrorMessage(error)}`)
  }
}
