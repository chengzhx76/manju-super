...
const generateVideoAsync = async (
  ...
  const useReferenceArray = resolvedModelName
    .toLowerCase()
...
const normalizeEndpoint = (
  endpoint?: string,
  fallback: string = VOLCENGINE_TASK_DEFAULT_ENDPOINT
): string => {
...
const generateVideoVolcengineTask = async (
  ...
  if (endImageBase64) {
    console.warn(
      '⚠️ Volcengine task mode currently uses start-frame only. End frame will be ignored.'
    )
  }
...
export const generateVideo = async (
  ...
  const isSoraCompatibleModel = isSoraCompatibleVideoModel(
