...
const SCENE_SCOPED_SHOT_PATTERN =
  /^scene[-_]?0*(\d+)[-_]shot[-_]?(\d+)(?:[-_](\d+))?$/i
...
export const sceneIdsMatch = (left: string, right: string): boolean => {
  ...
  return (
    rawLeft === rawRight ||
    normalizeSceneId(rawLeft) === normalizeSceneId(rawRight)
  )
}
...
export const parseShotId = (value: string): ParsedShotId => {
  ...
  if (sceneScopedMatch) {
    ...
    const subIndex = sceneScopedMatch[3]
      ? Number(sceneScopedMatch[3])
      : undefined
...
export const getShotDisplayKey = (
  shotId: string,
...
export const getShotDisplayLabel = (
  shotId: string,
  fallbackIndex: number
): string => `SHOT ${getShotDisplayKey(shotId, fallbackIndex)}`
  ...
  if (anchor.mode === 'canonical') {
    const maxSubIndex = shotIds.reduce((max, currentShotId) => {
      ...
      if (
        ...
        parsed.mainIndex !== anchor.mainIndex ||
...
export const shotBelongsToGroup = (
  shotId: string,
  ...
  if (shot.mode === 'scene-scoped' && group.mode === 'scene-scoped') {
    return (
      shot.sceneIndex === group.sceneIndex && shot.shotIndex === group.shotIndex
    )
  }
