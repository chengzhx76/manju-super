...
interface LegacyProjectState {
  ...
  scriptData: {
    ...
    visualStyle?: string
    shotGenerationModel?: string
    artDirection?: ScriptData['artDirection']
    characters: Character[]
    scenes: Scene[]
    props: Prop[]
    storyParagraphs: ScriptData['storyParagraphs']
  } | null
  shots: LegacyShotLike[]
  isParsingScript: boolean
  renderLogs: RenderLog[]
}

const buildDefaultEpisodeTitle = (episodeNumber: number): string =>
  `第 ${episodeNumber} 集`
...
export async function runV2ToV3Migration(db: IDBDatabase): Promise<void> {
  ...
  const legacyProjects = await new Promise<LegacyProjectState[]>(
    (resolve, reject) => {
  ...
  const tx = db.transaction(
    ['seriesProjects', 'series', 'episodes'],
    'readwrite'
  )
  ...
  for (const legacy of legacyProjects) {
    try {
      ...
      if (legacy.scriptData && !legacy.scriptData.props)
        legacy.scriptData.props = []
      ...
      const libraryChars: Character[] = characters.map((c) => ({
        ...c,
        version: 1
      }))
      const episodeChars: Character[] = characters.map((c) => ({
        ...c,
        libraryId: c.id,
        libraryVersion: 1
      }))
      const libraryScenes = scenes.map((s) => ({ ...s, version: 1 }))
      const episodeScenes = scenes.map((s) => ({
        ...s,
        libraryId: s.id,
        libraryVersion: 1
      }))
      const libraryProps = props.map((p) => ({ ...p, version: 1 }))
      const episodeProps = props.map((p) => ({
        ...p,
        libraryId: p.id,
        libraryVersion: 1
      }))
      ...
      const ep: Episode = {
        ...
        shotGenerationModel:
          normalizeChatModelId(legacy.shotGenerationModel) || 'gpt-5.2',
        scriptData: legacy.scriptData
          ? {
              ...legacy.scriptData,
              characters: episodeChars,
  ...
  await new Promise<void>((resolve, reject) => {
    tx.oncomplete = () => {
      console.log(
        `Migration complete: ${legacyProjects.length} projects migrated.`
      )
      resolve()
    }
...
export async function runEpisodeTitleFixMigration(
  db: IDBDatabase
): Promise<void> {
  if (!db.objectStoreNames.contains('seriesProjects')) return
  ...
  console.log(
    `Episode title fix migration applied: ${updates.length} episode(s) updated.`
  )
}
