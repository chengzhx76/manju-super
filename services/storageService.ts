...
const normalizeEpisode = (ep: Episode): Episode => {
  ...
  return {
    ...
    characterRefs: mergeByKey(
      ep.characterRefs,
      inferredCharacterRefs,
      (r) => r.characterId
    ),
    ...
    promptTemplateOverrides: sanitizePromptTemplateOverrides(
      ep.promptTemplateOverrides
    )
  }
}
...
export const getSeriesByProject = async (
  projectId: string
): Promise<Series[]> => {
  const db = await openDB()
  return new Promise((resolve, reject) => {
...
export const getEpisodesByProject = async (
  projectId: string
): Promise<Episode[]> => {
  const db = await openDB()
  return new Promise((resolve, reject) => {
...
export const getAllProjectsMetadata = async (): Promise<ProjectState[]> => {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.onsuccess = () => {
      const eps = ((req.result as ProjectState[]) || []).map((ep) =>
        normalizeEpisode(ep as Episode)
      )
...
export const saveAssetToLibrary = async (
...
export const getAllAssetLibraryItems = async (): Promise<
  AssetLibraryItem[]
> => {
  const db = await openDB()
  return new Promise((resolve, reject) => {
...
export const exportIndexedDBData =
  async (): Promise<IndexedDBExportPayload> => {
    const db = await openDB()
    return new Promise((resolve, reject) => {
        ...
        void (async () => {
          try {
            ...
            const portableEpisodes = await Promise.all(
              episodes.map((ep) =>
                materializeProjectVideosForExport(normalizeEpisode(ep))
              )
            )
...
): Promise<IndexedDBExportPayload> => {
...
export const exportSeriesProjectData = async (
  projectId: string
): Promise<IndexedDBExportPayload> => {
  const db = await openDB()
  return new Promise((resolve, reject) => {
    ...
    tx.oncomplete = () => {
      void (async () => {
        ...
        try {
          const rawEpisodes = ((epReq.result as Episode[]) || []).map(
            normalizeEpisode
          )
          const portableEpisodes = await Promise.all(
...
const isValidExportPayload = (
  data: unknown
): data is IndexedDBExportPayload => {
...
export const importIndexedDBData = async (
  ...
  const importedEpisodes = await Promise.all(
    ((payload.stores.episodes || []) as Episode[]).map(async (ep) => {
      try {
        ...
        console.warn(
          'Failed to normalize imported episode video storage. Keep original payload.',
          error
        )
  ...
  return new Promise((resolve, reject) => {
    ...
    ;(payload.stores.seriesProjects || []).forEach((sp: SeriesProject) => {
      spStore.put(sp)
      count++
    })

    const seriesStr = tx.objectStore(SERIES_STORE)
    ;(payload.stores.series || []).forEach((s: Series) => {
      seriesStr.put(s)
      count++
    })

    const epStore = tx.objectStore(EP_STORE)
    importedEpisodes.forEach((ep: Episode) => {
      epStore.put(ep)
      count++
    })

    if (
      payload.stores.projects &&
      payload.stores.projects.length > 0 &&
      !(payload.stores.episodes && payload.stores.episodes.length > 0)
    ) {
      payload.stores.projects.forEach((p: ProjectState) => {
        ...
        const episodeScenes = scenes.map((s: Scene) => ({
          ...s,
          libraryId: s.id,
          libraryVersion: 1
        }))
        const episodeProps = props.map((pr: Prop) => ({
          ...pr,
          libraryId: pr.id,
          libraryVersion: 1
        }))
        ...
        const sp: SeriesProject = {
          ...
          characterLibrary: chars.map((c: Character) => ({ ...c, version: 1 })),
          sceneLibrary: scenes.map((s: Scene) => ({ ...s, version: 1 })),
          propLibrary: props.map((pr: Prop) => ({ ...pr, version: 1 }))
        }
        spStore.put(sp)
        ...
        const ep: Episode = {
          ...
          visualStyle: p.visualStyle || '3d-animation',
          shotGenerationModel:
            normalizeChatModelId(p.shotGenerationModel) || 'gpt-5.2',
          scriptData: p.scriptData
            ? {
                ...p.scriptData,
                characters: chars.map((c: Character) => ({
                  ...c,
                  libraryId: c.id,
                  libraryVersion: 1
                })),
          ...
          characterRefs: chars.map((c: Character) => ({
            characterId: c.id,
            syncedVersion: 1,
            syncStatus: 'synced' as const
          })),
          sceneRefs: scenes.map((s: Scene) => ({
            sceneId: s.id,
            syncedVersion: 1,
            syncStatus: 'synced' as const
          })),
          propRefs: props.map((pr: Prop) => ({
            propId: pr.id,
            syncedVersion: 1,
            syncStatus: 'synced' as const
          })),
          promptTemplateOverrides: sanitizePromptTemplateOverrides(
            p.promptTemplateOverrides
          ),
          scriptGenerationCheckpoint: null
        }
    ...
    tx.oncomplete = () =>
      resolve({
        projects: count,
        assets: (payload.stores.assetLibrary || []).length
      })
    tx.onerror = () => reject(tx.error)
  })
}
